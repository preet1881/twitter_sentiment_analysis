...
class SentimentAnalysis():
    ...
    def DownloadData(self, username, worksheet_list, start_date, end_date, list_variable):
        #Twitter credentials for the app
        consumer_key = "L0QtOdlCAS0Cq5lL0tFasxUk6"
        consumer_secret = "jkYPzxSNgQG9X5U2Sz8ofH3aaOlzR5Aa209pv4fQCmNKRVokIV"
        access_key= "900989333313732613-gATs10bME6XsfK9BcP6VLEFFF0h9KEa"
        access_secret = "RsrgC3HspjbgEPLucnpAk9gqEYRPwV9iRQG3XI2lPfAE0"
    ...
    def sentiment_analysis(self, tweets, username, worksheet_list, start_date, end_date, list_variable):
        ...
        for tweet in tweets:
            ...
            new_stopwords=['amp', 'ji', 'shree','rs','shri', 'rt','mr']
...
def plot_with_multiple_axes(username,x_value, y_value, x_value2, y_value2,x_value3, y_value3, xlabel, ylabel, xlabel2, ylabel2, linename, linename2,linename3, plotname, ymin , ymax):
    ...
    ax.plot(x_value, y_value, color="C0", label=linename)
...
def single_axis_plotter(xvalue, yvalue, yavg, username, date_list,names, xlabel, ylabel, fig_name, ymin, ymax):
    ...
    for i in range(0, len(username)):
        ax.plot(xvalue[i], yvalue[i], color=color[i], label=names[i])
        if i==0:
            ax.plot(xvalue[0], yavg, color='black', label='Average for all' )
...
def multiple_user_multiple_axes(xvalue, yvalue, yvalue2,y2linename, username, date_list,names, xlabel, ylabel, ylabel2, fig_name, ymin, ymax):
    fig=plt.figure(figsize = (20, 10))
    ax=fig.add_subplot(111, label="1")
    ...
    for i in range(0, len(username)):
        ax.plot(xvalue[i], yvalue[i], color=color[i], label=names[i])
        if i==0:
            ax11.plot(xvalue[0], yvalue2, color='black', label=y2linename)
    color2=['yellow', 'white','cyan', 'yellow', 'white', 'cyan' ]
    for i in range(0,len(date_list)-1):
        s, e= get_data_csv(xvalue[0], date_list[i],date_list[i+1])
        ax.axvspan(xvalue[0][s],xvalue[0][e] , facecolor=color2[i], alpha=0.2)
...
def average_comparison_plot(user_dict, start_date, end_date, state_comp, individual_comp, date_list):
    ...
    single_axis_plotter(dates_all, avg, average_all, username, date_list, names ,"Days", "Average", "Average Plot", -1.1, 1.1)
    ...
    for i in range(p, q):
        ...
        total_deceased.append(file.totaldeceased[i])
        daily_deceased.append(file.dailydeceased[i])
    
    #multiple_user_multiple_axes(xvalue, yvalue, yvalue2, y2linename,username, date_list,names, xlabel, ylabel, ylabel2, fig_name, ymin, ymax)
    multiple_user_multiple_axes(dates_all, day_polarity, daily_deceased, "Daily Deaths",username, date_list,names, "Days", "Polarity per day", "Number of cases", "Death vs Polarity per day Plot", -1.1, 1.1)
    multiple_user_multiple_axes(dates_all, avg, daily_deceased, "Daily Deaths",username, date_list,names, "Days", "Average per day", "Number of cases", "Daily Deaths vs Average Plot", -1.1, 1.1)
    ...
    if state_comp!="True" and individual_comp=="True":
        for i in range(0, len(username)):
...
def state_plots(user_dict, state_list, start_date, end_date):
    ...
    #plot_with_multiple_axes(username,x_value, y_value, x_value2, y_value2,x_value3, y_value3, xlabel, ylabel, xlabel2, ylabel2, linename, linename2,linename3, plotname, ymin , ymax):
    for i in range(0, len(username)):
        plot_with_multiple_axes(state_list[i], dates_all[i], day_polarity[i], dates_all[i], state_corona_cases_daily[i],dates_all[i], state_daily_deceased[i],"days","polarity per day",  "days","Cases", username[i], "Daily Cases","Daily Deaths", "Polarity vs daily_cases for ", -1.1, 1.1 )
        plot_with_multiple_axes(username[i], dates_all[i], sum[i], dates_all[i], state_corona_cases_daily[i], dates_all[i], state_daily_deceased[i], "days","polarity sum","days", "Cases",  username[i], "Daily Cases","Daily Deaths", "Sum vs daily_deceased for ", 0, 0 )
        plot_with_multiple_axes(username[i], dates_all[i], avg[i], dates_all[i], state_corona_cases_daily[i], dates_all[i], state_daily_deceased[i], "days","Average polarity","days", "Cases",  username[i], "Daily Cases","Daily Deaths", "Average vs daily_deceased for", -1.1, 1.1 )
...
if __name__== "__main__":
   print("Enter Start Date After 14/3/20: ")
   start_date=ObtainDate()
   print("Start Date ",start_date)
   print("Enter End Date Before 7/6/20:")
   ...
   for i in range(0, len(username)):
       sa.DownloadData(username[i], worksheet_list, start_date, end_date,i)
