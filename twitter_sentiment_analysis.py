...
def average_comparison_plot(user_dict, start_date, end_date, state_comp, individual_comp, date_list):
    ...
    sensex_file=pd.read_csv('Sensex_data.csv', index_col=False)
    money_date=sensex_file.Date.tolist()
    closing_value=sensex_file.Close.tolist()
    open_value=sensex_file.Open.tolist()
    multiple_user_multiple_axes(dates_all, day_polarity, money_date, closing_value, "Sensex Closing Values", username, date_list, names, "Days", "Polarity per day", "Closing Values", "Sensex vs Polarity", -1.1, 1.1)
    ...
    if individual_comp=="True":
        for i in range(0, len(username)):
               ...
               plot_with_multiple_axes(username[i], dates_all[i], day_polarity[i], money_date, closing_value, money_date, open_value, "Dates", "Polarity per Day", "Days", "Points", username[i], "Sensex Closing Values", "Sensex Open Values", "Polarity vs Sensex", -1.1, 1.1)
               plot_with_multiple_axes(username[i], dates_all[i], sum_pol[i], money_date, closing_value, money_date, open_value, "Dates", "Sum of Polarity of Tweets", "Days", "Points", username[i], "Sensex Closing Values", "Sensex Open Values", "Sum vs Sensex", 0,0)
               plot_with_multiple_axes(username[i], dates_all[i], avg[i], money_date, closing_value, money_date, open_value, "Dates", "Avg of Polarity of Tweets", "Days", "Points", username[i], "Sensex Closing Values", "Sensex Open Values", "Avg vs Sensex", -1.1,1.1)
